...
const JSLVal=``
 ...
 for (let i =2; i <= Count; i++) {
	...
	if (QK==0&&JSAVal!='') {
	console.log($.name+',Cookie批量导入成功')
			 $.msg($.name+',Cookie批量导入成功')
	
	   }
	
	if (QK==1) {
	console.log($.name+',Cookie清除成功')
			 $.msg($.name+',Cookie清除成功')
...
if (!jsAArr[K-1]) {
if (K>R)
    $.msg($.name+R+'到'+(K-1)+'打印成功'+K+'到'+Z+'打印失败', '⚠️提示：请先获取'+K+'到'+Z+'的Cookie','并设置好账号数')
if (K==R&&Z>R)
    $.msg($.name+K+'到'+Z+'打印失败', '⚠️提示：请先获取'+K+'到'+Z+'的Cookie','并设置好账号数')
if (Z==R)
    $.msg($.name+K+'打印失败', '⚠️提示：请先获取'+K+'的Cookie','并设置好账号数')
   $.done();
  }
  jsAVal = jsAArr[K-1];  
  jsBVal = jsBArr[K-1];  
